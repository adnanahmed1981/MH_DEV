...
if (Yii::app()->user->member->step == 999) {
    if (Yii::app()->user->member->subscription_end_date == '0000-00-00 00:00:00') {
        // Never Paid
        
    } else if (strtotime(Yii::app()->user->member->subscription_end_date) < time()) {
        // Paid but expired
...
<div class="container">
	<div class="row">
		<div class="col-md-12">
 		<h3><i class="fa fa-arrow-circle-up" aria-hidden="true"></i> <?php echo $title; ?></h3>
		</div>
	</div>
	<hr class="hr-small">
	...
	<div class="row md-margin-top">
		<div class="col-sm-offset-0 col-sm-6 col-md-offset-1 col-md-5">
        		...
        		$form = $this->beginWidget('CActiveForm', array('id' => 'upgrade-form',
					'action' => Yii::app()->createUrl('//site/upgrade'),
...
$(document).ready(function() {
	...
	$('.apply_btn').click(
		function() {
			$.ajax({async: false,
				url: base_url + 'index.php/dynamicAjax/dynamicGetCoupons',
				type: 'POST',
				dataType: 'json',
			...
			return false;
                ...
                $('#apply-coupon').click(function() {
                    $('#freetrialdesc').hide();
                    $('#btnloader').show();
                    var pakcageflag = false;
                    var CouponCode = $("#CheckoutForm_CouponCode").val();
                    if (CouponCode != "") {
                        $.ajax({async: false,
                            url: base_url + 'index.php/dynamicAjax/getCouponPackage',
                            type: 'POST',
                            dataType: 'json',
                            ...
                            success:
                                    function(result) {
                                        if (result['package'] != "") {
                                            ...
                                            alertify.alert("Coupon code '" + CouponCode + "' doesn't exist!");
                                            setTimeout(function() {
                        ...
                        alertify.alert("Please enter a valid Coupon code!");
                        setTimeout(function() {
                            $(".alertify-button-ok").trigger("click");
                        }, 5000);
                        pakcageflag = false;
